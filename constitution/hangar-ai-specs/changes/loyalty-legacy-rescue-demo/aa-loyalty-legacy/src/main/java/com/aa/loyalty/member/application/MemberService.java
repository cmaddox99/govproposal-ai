...
import com.aa.loyalty.events.MemberEnrolledEvent;
 ...
 */
@Service
public class MemberService implements MemberSelfServicePort {
    ...
    public Member enroll(String firstName, String lastName, String email,
        ...
        eventPublisher.publishEvent(new MemberEnrolledEvent(saved.getMemberNumber(), saved.getEmail(), saved.getEnrollmentDate()));
        return saved;
    }

    @Transactional
    public Member updateProfile(String memberNumber, String email, String phone, Address address) {
    ...
    public void updateTierStatus(String memberNumber, String newTier) {
        ...
        TierStatus oldTier = member.getTierStatus();
        member.setTierStatus(TierStatus.valueOf(newTier));
        member.setLastUpdated(new Date());
    ...
    // ISP FIX: The 4 UnsupportedOperationException methods (bulkImportMembers,
    // exportMemberDataForRegulator, freezeAccountForFraud, archiveMemberForCompliance)
    // have been removed. MemberService now implements only MemberSelfServicePort,
    // which covers the self-service bounded context it actually owns.
}
